...
import com.example.usermanagementservice.services.AuthService;
...
class AuthServiceTest {
    ...
    void updateProfile_invalidToken_throwsUnauthorizedException() {
        ...
        assertThrows(UnauthorizedException.class,
                () -> authService.updateProfile(2L, "bad", "New Name", null, null));
    }
    ...
    void updateProfile_success_updatesNameAndPhone() {
        ...
        User result = authService.updateProfile(2L, token, "New Name", "9999999999", "221B Baker Street");

        assertEquals("New Name", result.getName());
        assertEquals("9999999999", result.getPhoneNumber());
        assertEquals("221B Baker Street", result.getAddress());
    }
    ...
    void forgotPassword_success_savesTokenAndSendsEmail() throws JsonProcessingException {
        ...
        String resetToken = authService.forgotPassword("u@example.com");

        assertNotNull(resetToken);
        verify(passwordResetTokenRepo).save(argThat(t -> !t.isUsed() && t.getExpiresAt().after(new Date()) && t.getToken().equals(resetToken)));
