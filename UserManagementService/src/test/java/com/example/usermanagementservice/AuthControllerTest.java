...
import com.example.usermanagementservice.controllers.AuthController;
...
class AuthControllerTest {
    ...
    void updateProfile_success_returnsUpdatedUserDto() throws Exception {
        ...
        when(authService.updateProfile(eq(1L), eq("valid-token"), anyString(), anyString(), anyString())).thenReturn(user);

        UpdateProfileRequestDto body = new UpdateProfileRequestDto();
        body.setName("Alice Updated");
        body.setPhoneNumber("9999999999");
        body.setAddress("221B Baker Street");
    ...
    void forgotPassword_success_returns200() throws Exception {
        when(authService.forgotPassword("alice@example.com")).thenReturn("test-reset-token");
